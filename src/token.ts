...
import { ERC20, Transfer } from "../generated/Noob/ERC20";
import {
  decreaseAccountBalance,
  getOrCreateAccount,
  getOrCreateAccountBalance,
  increaseAccountBalance, isNewAccount
} from "./account";
...
export function handleTransfer(event: Transfer): void {
  ...
  if (token == null) {
    token = new Token(Bytes.fromHexString(event.address.toHex()));
    token.name = "NOOB";
    token.symbol = "NOOB";
    token.decimals = 18;
